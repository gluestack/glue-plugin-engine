---
title: CLI Reference | engine
description: CLI Reference | engine
---

import { Canvas, Meta, Story } from "@storybook/addon-docs";

<Meta title="Engine Plugin/CLI Reference" />
...
> **Note:** 
> 1. Please don't run `node glue develop:up` again if your gluestack project is already running. As our "**Engine**" will notice that the ports all instances were suppose to run are already in-use and will reassign all instances with newer ports.
> 2. In case, you want to run your gluestack with frequent changes, we advice to use `node glue develop:watch` command. This command brings down the instances and brings them up again with the latest changes.
